...
$cartIcon = '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-cart-check-fill" viewBox="0 0 16 16">
 ...
 * @return array
 */
function getNamesFromId(mysqli $connection, string $tableName, string $idField = "id", string $nameField = "name"): array
{
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($row = $result->fetch_assoc()) {
            $names[$row[$idField]] = $row[$nameField];
        }
    }
    return $names;
}
